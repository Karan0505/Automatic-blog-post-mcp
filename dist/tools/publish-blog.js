...
export const PUBLISH_BLOG_TOOL_DEFINITION = {
    ...
    description: 'Publish a reviewed blog article to the connected Strapi CMS and Next.js website. ' +
...
export async function handlePublishBlog(client, args) {
    try {
        const parsed = z.object(publishBlogSchema).parse(args);
        const result = await client.publishBlog(parsed);
        if (!result.ok) {
            return {
                content: [
                    {
