...
export const SEARCH_TOPIC_IMAGE_TOOL_DEFINITION = {
  ...
  description:
    'Search topic-specific cover images and generate an editorial image prompt tailored for a blog post topic. ' +
...
export async function handleSearchTopicImage(args: any) {
  try {
          ...
          text: JSON.stringify(
            {
              ...
              instructions:
                'Pass `imageUrl: suggestedCoverImageUrls[0]` or `imagePrompt: recommendedImagePrompt` into `publish_blog_to_strapi` to apply this topic-specific image to your blog post.',
            },
            null,
            2
          ),
